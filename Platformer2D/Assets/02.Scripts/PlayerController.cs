...
public class PlayerController : MonoBehaviour
{
    ...
    // 플레이어의 상태가 바뀔때 초기화 해야하는 요소들을 초기화해준다. 
            ...
            case AttackState.Attacking:
                if (animationTimeElapsed > attackTime)
                {
                    RaycastHit2D hit = Physics2D.BoxCast
                        (rb.position + attackCastingCenter, attackCastingSize, 0f, attackCastingDirection, attackTargetLayer);
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireCube(new Vector3(transform.position.x + attackCastingCenter.x, transform.position.y + attackCastingCenter.y, 0f),
